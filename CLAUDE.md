...
Domain vocabulary: a **portfolio** owns **sleeves** (`core` = passive ETF, `satellite` = stock picks) holding **positions**; **fact tables** (instruments, prices_daily, funds, fund_holdings, fundamentals) are shared read-all, **portfolio tables** are RLS-scoped via `SET LOCAL app.portfolio_id`. The **ingestion pipeline** (adapter registry → pydantic contracts → quarantine → loaders, checksummed `ingest_runs`) writes fact tables with the admin engine; the API uses the RLS-enforced app engine. Its expanded source covers 1,000 stocks plus 200 ETFs; latest-year SEC values supplement missing comparison fields, and sparse fundamentals upserts preserve existing non-null facts. Analytics (projections, magic formula, TER drag, technical indicators SMA/EMA/RSI/MACD, and the 6-criterion **portfolio health radar** on `/summary`, whose **volatility** and **sector-concentration** criteria are full-portfolio **look-through**: the core ETF is decomposed into its `fund_holdings` basket (sector concentration falls back to the fund's `category` when a fund discloses no holdings), unpriced names keep their weight via a beta × market-factor proxy, and the surviving priced names are never rescaled to 100% — so a concentrated or undiversified core actually counts instead of being invisible) are always computed on the fly — no stored derived tables. Fund comparison: `GET /api/market/funds?compare=A,B`. The **grounded LLM features** — stock **comparison** (`/api/compare`) and single-stock **analysis** (`/api/analysis/stock`) — share `services/grounding.py`: facts injected as vertical key-value blocks rounded to 6 significant figures, output numbers guarded against the injected set, one retry, audited in `llm_audit_log`. The **copilot** (right drawer) chats over a **LangGraph agent** (`services/agent/`): `scope_guard → planner → executor → synthesiser → grounding validator`, up to 3 self-correcting re-plans (each validation error — fabricated figure, failed run_sql, flagged miss — is fed back to the planner) then a single `rag_search` fallback then honest refusal; planner SQL is guided by the vetted patterns in `services/agent/sql_templates.py` (one source of truth, executed by `test_sql_templates`); LLM calls in exactly those three LLM nodes via the `AgentLLM` protocol (`PydanticOutputParser` prompting — the small local model ignores tool-calling/json_schema modes). Executor tools: `run_sql` (planner-written SELECT on the RLS-scoped, READ ONLY, timeout-capped connection; sanitized first — ```sql fences stripped, only the first statement kept, placeholder tokens like `<TICKER>` and unnormalized `instruments.sector` predicates rejected — so small-model drift never reaches the DB), `get_projection` (deterministic analytics), and `rag_search` (deterministic embed → hybrid pgvector∪full-text pool → cross-encoder rerank over the shared `doc_chunks` fact table; the step's question is the query). Chat: `POST /api/portfolios/{id}/chat` (SSE: plan → evidence → answer), history GET, clear-context DELETE, audit GET, `GET /api/copilot/info` (model name); `chat_messages` is RLS-scoped; `llm_audit_log` carries `graph_run_id` + `node` per copilot run. Canned refusal texts are filtered out of the LLM context (a small model treats past refusals as precedent and keeps refusing). The **RAG branch has landed**: PDFs ingest through the same pipeline (`pdf` adapter → per-page pypdf chunks → nomic-embed-text 768-d embeddings → `doc_chunks`, a shared read-all fact table with pgvector HNSW + generated full-text indexes, checksum-idempotent), and both agents plan over the shared `rag_search` tool; an empty corpus simply yields a gap. A second **draft agent** (`services/agent/draft_service.py`, `POST /api/portfolio-draft/chat`) reuses the same `GroundedAgent` core to build a portfolio from natural language and create it (on explicit confirmation) through `PortfolioService.create` (satellite names/tickers are resolved against `instruments` by ticker, exchange-suffix, or company name; if none resolve it refuses to propose a lone 100%-core draft); provider per agent is config (`COPILOT_PROVIDER`/`DRAFT_AGENT_PROVIDER` = ollama|openai) via `services/agent/provider.py`. See `docs/ARCHITECTURE.md` §4.

Key extra commands: `just stack-up` (Postgres :5434 + Adminer web UI :8080), `just db-apply`, `just ingest-all [data_dir]`. Tests run against the dedicated `coresat_test` database and auto-skip without Postgres; the real-LLM comparison test is gated behind `CORESAT_REAL_LLM=1` (needs Ollama serving `gemma4:e4b`).

Before anything on a dev laptop: verify RTK is active (`rtk --version && rtk gain`). See `~/.claude/RTK.md`.

## Project Layout

- `projects/backend/coresat` — FastAPI app: `main.py` entrypoint, `api/` routers, `core/config.py` settings; `projects/backend/tests`

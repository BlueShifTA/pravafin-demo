...
- **grounding_validator** (plain code): every numeric claim must appear in SQL rows / projections / chunks → any validation problem (fabricated figure, failed run_sql, flagged miss) injects errors → planner re-plans with the error fed back, up to 3 attempts → still failing → one `rag_search` fallback over the document corpus → still failing → "cannot answer from data". Planner SQL is guided by the vetted patterns in `services/agent/sql_templates.py`.

"""Portfolio endpoints: wizard create, selector list, summary."""

from fastapi import APIRouter, HTTPException, Request

import coresat.domain as csd
import coresat.services as css

router = APIRouter(prefix="/portfolios", tags=["portfolios"])


def _service(request: Request) -> css.PortfolioService:
    service: css.PortfolioService = request.app.state.portfolio_service
    return service


def _analytics(request: Request) -> css.AnalyticsService:
    service: css.AnalyticsService = request.app.state.analytics_service
    return service


@router.post("", status_code=201)
async def create_portfolio(spec: csd.PortfolioCreate, request: Request) -> csd.PortfolioCreated:
    try:
        portfolio_id = await _service(request).create(spec)
    except css.UnknownTickerError as error:
        raise HTTPException(status_code=422, detail=str(error)) from error
    return csd.PortfolioCreated(id=portfolio_id)


@router.get("")
async def list_portfolios(request: Request) -> list[csd.PortfolioListItem]:
    return await _service(request).list()


@router.get("/{portfolio_id}/summary")
async def portfolio_summary(portfolio_id: int, request: Request) -> csd.PortfolioSummary:
    summary = await _analytics(request).summary(portfolio_id)
    if summary is None:
        raise HTTPException(status_code=404, detail="portfolio not found")
    return summary

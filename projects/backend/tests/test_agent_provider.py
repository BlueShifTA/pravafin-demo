...
def _settings(**overrides: object) -> csc.Settings:
...
def test_openai_provider_without_key_fails_loud() -> None:
    with pytest.raises(ValueError, match="OPENAI_API_KEY"):
        csa.build_chat_model("openai", _settings(openai_api_key=""))


def test_unknown_provider_fails_loud() -> None:
    with pytest.raises(ValueError, match="unknown LLM provider"):
        csa.build_chat_model("gemini", _settings())

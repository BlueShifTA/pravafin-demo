...
from coresat.services.agent.tools import _prepare_sql
...
def _sql_step(sql: str | None) -> csd.Step:
    return csd.Step(id=1, question="q", tool=csd.ToolName.RUN_SQL, sql=sql)
...
class _FakeSummaries:
    def __init__(self, summary: csd.PortfolioSummary | None) -> None:
        self._summary: csd.PortfolioSummary | None = summary

    async def summary(self, portfolio_id: int) -> csd.PortfolioSummary | None:
        return self._summary


def _summary(portfolio_id: int) -> csd.PortfolioSummary:
    return csd.PortfolioSummary(
        ...
        drift=[],
        projections=[
            csd.ProjectionOut(years=10, annual_rate=0.07, expected=152340.55, low=1e5, high=2e5),
        ],
        health=csd.PortfolioHealth(headline=0.0, criteria=[]),
    )


async def test_get_projection_renders_summary_numbers() -> None:
    tool = csa.GetProjectionTool(summaries=_FakeSummaries(_summary(1)), portfolio_id=1)
    evidence = await tool.run(
        csd.Step(id=1, question="projection?", tool=csd.ToolName.GET_PROJECTION)
    )
    assert evidence.error is None
...
async def test_get_projection_missing_portfolio_is_error() -> None:
    tool = csa.GetProjectionTool(summaries=_FakeSummaries(None), portfolio_id=99)
    evidence = await tool.run(
        csd.Step(id=1, question="projection?", tool=csd.ToolName.GET_PROJECTION)
    )
    assert evidence.error is not None

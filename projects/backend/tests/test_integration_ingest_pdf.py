...
async def pipeline() -> AsyncIterator[csi.IngestionPipeline]:
    ...
    yield csi.IngestionPipeline(engine=engine, registry=csi.build_registry(FakeEmbedder()))
    await engine.dispose()


async def test_pdf_ingests_chunks_with_page_provenance(pipeline: csi.IngestionPipeline) -> None:
...
async def test_long_page_splits_into_ordered_chunks(pipeline: csi.IngestionPipeline) -> None:
...
async def test_reingest_same_pdf_skips_no_duplicates(pipeline: csi.IngestionPipeline) -> None:
...
async def test_reingest_after_interrupted_run_resumes(pipeline: csi.IngestionPipeline) -> None:
...
async def test_pdf_without_source_ref_is_rejected(pipeline: csi.IngestionPipeline) -> None:
    with pytest.raises(ValueError, match="source_ref"):
        await pipeline.run("pdf", make_text_pdf(["x"]), None)


async def test_modified_pdf_updates_chunk_in_place(pipeline: csi.IngestionPipeline) -> None:

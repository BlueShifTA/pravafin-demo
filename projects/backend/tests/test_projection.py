...
def test_lump_sum_only_compounds() -> None:
    result = css.project(capital=10_000, monthly_contribution=0, annual_rate=0.07, years=10)
    assert result.expected == pytest.approx(10_000 * 1.07**10, rel=1e-9)


def test_contributions_accumulate() -> None:
    result = css.project(capital=0, monthly_contribution=100, annual_rate=0.05, years=10)
    annual = 1200
...
def test_zero_rate_is_linear() -> None:
    result = css.project(capital=1_000, monthly_contribution=100, annual_rate=0.0, years=5)
    assert result.expected == pytest.approx(1_000 + 100 * 12 * 5, rel=1e-9)


def test_band_is_plus_minus_one_percent() -> None:
    result = css.project(capital=10_000, monthly_contribution=0, annual_rate=0.07, years=20)
...
def test_negative_years_rejected() -> None:
    with pytest.raises(ValueError):
        css.project(capital=1, monthly_contribution=0, annual_rate=0.05, years=-1)
